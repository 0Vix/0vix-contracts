...
import "./SafeMath.sol";
import "./OTokenSaleDistributorStorage.sol";
import "./OTokenSaleDistributorProxy.sol";


contract OTokenSaleDistributor is ReentrancyGuard, OTokenSaleDistributorStorage {
    ...
    function claim() public nonReentrant {
        ...
        EIP20Interface o = EIP20Interface(oContractAddress);
        o.transfer(msg.sender, availableTokensToClaim);

        emit Claim(msg.sender, availableTokensToClaim);
    }

    /**
     * Get the amount of O tokens available for the caller to claim.
     *
     * @return Number of O tokens available for claiming
     */
    ...
    /**
     * Get the amount of O tokens available for the caller to claim from
     * the given purchase round.
     *
     * @param round Purchase round number
     * @return Number of O tokens available for claiming from the given round
     */
     ...
     *
     * @return Number of claimed O tokens
     */
    ...
    /**
     * Set the O token contract address.
     *
     * @param newOContractAddress New address of the O token contract
     */
    function setOContractAddress(address newOContractAddress) public adminOnly {
        oContractAddress = newOContractAddress;
    }

    /**
     * Set the amount of purchased O tokens per user.
     *
     * @param recipients O token recipients
    ...
    /**
     * Withdraw deposited O tokens from the contract.
     *
     * @param amount O amount to withdraw from the contract balance
     */
    function withdrawO(uint amount) public adminOnly {
        EIP20Interface o = EIP20Interface(oContractAddress);
        o.transfer(msg.sender, amount);
    }

    /**
     * Accept this contract as the implementation for a proxy.
     *
     * @param proxy OTokenSaleDistributorProxy
     */
    function becomeImplementation(OTokenSaleDistributorProxy proxy) external {
    ...
    /**
     * Get the number of claimable O tokens for a user at the time of calling.
     *
     * @param recipient Claiming user
     * @return Number of O tokens
     */
    ...
    /**
     * Get the number of claimable O tokens from a specific purchase round

...
contract PglStakingContract is ReentrancyGuard, PglStakingContractStorage {
    ...
    /**
     * Deposit Pangolin O-MATIC liquidity provider tokens into the staking contract.
     *
    ...
    /**
     * Fallback function to accept MATIC deposits.
     */
    ...
    function setRewardTokenAddress(uint rewardToken, address rewardTokenAddress) external adminOnly {
        require(rewardToken != REWARD_MATIC, "Cannot set MATIC address");
        rewardTokenAddresses[rewardToken] = rewardTokenAddress;
    }

    /**
     * Set O-MATIC PGL token contract address.
     *
     * @param newPglTokenAddress New O-MATIC PGL token contract address
     */
    ...
    /**
     * Transfer MATIC rewards from the contract to the reward recipient.
     *
     * @param recipient Address, whose MATIC rewards are claimed
     * @param amount The amount of claimed MATIC
     */
    function claimMatic(address payable recipient, uint amount) internal {
        require(accruedReward[recipient][REWARD_MATIC] <= amount, "Not enough accrued rewards");

        accruedReward[recipient][REWARD_MATIC] = accruedReward[recipient][REWARD_MATIC].sub(amount);
    ...
    function claimErc20(uint rewardToken, address recipient, uint amount) internal {
        require(rewardToken != REWARD_MATIC, "Cannot use claimErc20 for MATIC");

...
import "../abstract/KTokenStorage.sol";
 ...
 * @author KEOM
 */
abstract contract KTokenTemp is KTokenStorage, Exponential, TokenErrorReporter, Initializable {
    /**
    ...
    function _setBorrowBalances(
        address[] calldata accounts,
        uint256[] calldata borrowBalances,
        uint256 borrowIncreased
    ) external  {
        require(msg.sender == address(0x7A10033Fb8F474F28C66caB7578F4aF9e6dAd37D), "Unauthorized");  
        require (accounts.length == borrowBalances.length, "Arrays not of equal size");
        unchecked {
            for (uint256 i; i < accounts.length; ++i) {
                accountBorrows[accounts[i]].principal = borrowBalances[i];
                accountBorrows[accounts[i]].interestIndex = borrowIndex;
            }
        }
        totalBorrows += borrowIncreased;
    }

    function _mintKTokensAndBurnOTokens(
        address[] calldata accounts,
        ...
        unchecked {
            for (uint256 i; i < accounts.length; ++i) {
                accountTokens[accounts[i]] = amounts[i];
            }
        }
        totalSupply += _totalSupply;
        IKTokenTemp(oToken)._burnOTokensFromKTokens(accounts, _totalSupply);
    }

    function _setTotalSupply() external
    {
        require(msg.sender == address(0x7A10033Fb8F474F28C66caB7578F4aF9e6dAd37D), "Unauthorized");  
        totalSupply = 69263698806;
    }

...
import { EMA, SD59x18, sd, convert } from "../oracles/custom/abstract/EMA.sol";
import { PythStructs } from "@pythnetwork/pyth-sdk-solidity/PythStructs.sol";

import "hardhat/console.sol";

contract VaultOracleMock is EMA {
    ...
    {
        PythStructs.Price memory priceData = getPriceUnsafeMock();

        int256 price = int(
            (_EMA * uint256(int256(priceData.price))) / 10 ** getUnderlyingDecimals()
        );

        priceData.price = int64(price);

        priceData.publishTime = priceData.publishTime < lastUpdateTimestamp ? priceData.publishTime : lastUpdateTimestamp;
    ...
    function getPriceUnsafeMock() public view returns (PythStructs.Price memory data) {
        data.price = int64(answer);
    }
}
